...
void Processor::BuildDictorModels(const map<int, map<int, vector<vector<double>>>>& features) const {
...
void Processor::ExtractFeatures(const string& folder, const string& alignment_path, map<int, map<int, vector<vector<double>>>>& features, const string& file_path) {
	map<int, string> files;
	ReadRecordPaths(files, file_path);
	string line;
	int phoneme, record, dictor, prev_record = -1;
	int window_size_in_samples, hop_in_samples, sampling_rate, offset, start_in_samples, end_in_samples;
	double start, end;
	SoundFile sound;
	...
	if (f.is_open()) {
		while (getline(f, line)) {
			...
			start_in_samples = start * sampling_rate - offset;
			start_in_samples = std::max(start_in_samples, 0);
			end_in_samples = end * sampling_rate + offset;
...
void Processor::ReadFeatures(const string& alignment_path, map<int, map<int, vector<vector<double>>>>& features, const string& feature_path, const string& record_map_path) {
	string line;
...
Processor::Processor(const string& model_folder, const string& model_file) : windowSize(0.025), hop(0.01),
                                                                             gaussians(3), windowFunc(HammingWindow), featureSize(13), modelPath(model_folder + model_file), modelFolder(model_folder) {
